...
**Session 12: Certificate & Secure Onboarding Packaging**
    ...
    * **Action:** Write a utility script triggered by `provision_agent`. It generates the X.509 certificate and connection parameters. **Crucially, encrypt this entire package using a 6-digit One-Time PIN (OTP) generated by the server.** 
    * **Outcome:** You hand the operative the encrypted package, and send the 6-digit PIN via a separate secure channel (like Signal). Even if the package is intercepted on the network, the operative's keys remain completely secure.
